...
export const getBalance = async () => {
  ...
  if (result.error) {
    if (result.status_code === 400) throw APIError.InvalidRequest;
      ...
      return Loader.Cardano.Value.new(
        BigInt('0'),
...
export const getUtxos = async (amount = undefined, paginate = undefined) => {
  ...
  if (amount) {
    ...
    let filterValue;
    try {
      filterValue = Loader.Cardano.Value.from_cbor_bytes(
...
export const getCollateral = async () => {
  ...
  return utxos.filter(
    (utxo) =>
      utxo.output().amount().coin() <= BigInt('50000000') &&
...
export const createHWAccounts = async (accounts) => {
  ...
  accounts.forEach((account) => {
    ...
    const paymentKeyHash = Buffer.from(
      paymentKeyHashRaw.to_raw_bytes()
    ).toString('hex');
...
export const getBlazeProvider = async () => {
  ...
  const wallet = new WebWallet({
    getUtxos: async (amount, paginate) =>
      (await getUtxos(amount, paginate)).map((utxo) =>
        Buffer.from(utxo.to_cbor_bytes()).toString('hex')
      ),
